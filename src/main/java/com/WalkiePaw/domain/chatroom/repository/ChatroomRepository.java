...
public interface ChatroomRepository extends JpaRepository<Chatroom, Long>, ChatroomRepositoryOverride {
    ...
    @Query("select c from Chatroom c join Member m on m.id = :memberId where c.id = :crId")
    Optional<Chatroom> findWithMemberById(@Param("crId") final Long chatroomId, @Param("memberId") final Long memberId);
}
